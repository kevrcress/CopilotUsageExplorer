...
import type { ParsedSession, LlmCall, ToolCallSummary, RawEvent, Turn } from '@cue/core';
...
import { formatTokens, rollupSession, computeAiCredits } from '@cue/core';
import { modelTier, tierLabel, tierBadgeVariant } from '@cue/core';
import { formatDuration, getSessionDisplayTitle } from '@cue/core';
import { heuristicTokenCount } from '@cue/core';
import { AreaChart, Area, PieChart, Pie, Cell, ResponsiveContainer, Tooltip, XAxis, YAxis, CartesianGrid } from 'recharts';
import { exportSessionHtml, exportSessionJson } from '../export';
import { getHost } from '../store';
import { redactBody, redactString } from '@cue/core';
...
export function SessionDetail({ session, onBack }: { session: ParsedSession; onBack: () => void }) {
  ...
  const exportHtml = () => {
    void getHost().saver.save(`session-${session.id}.html`, exportSessionHtml(session, workspaceNames, redact), 'text/html;charset=utf-8');
  };
  const exportJson = () => {
    void getHost().saver.save(`session-${session.id}.json`, exportSessionJson(session, redact), 'application/json');
  };
